...
| `pose_probe.py` | `pose_probe` | Single self-contained REPL/one-shot tool (not part of the runtime pipeline): the pose analog of `home_config_tuner`. Builds its **own** in-process `CuRobo` planner (no curobo_service, no orchestrator). At a `pose>` prompt enter 7 numbers (`x y z qx qy qz qw`, tool0 in base_link) or a `place_poses.yml` key (`bookshelf`, `storage_1`…); it plans via `CuRobo.plan_trajectory` (single-pose), by default drives the arm so you can watch it reach the EEF in Gazebo, and reports the achieved tool0 pose with position/orientation error and a PASS/FAIL. `UNREACHABLE` if cuRobo finds no plan. `-p target:=<key>` runs one target and exits; `-p execute:=false` plans only. Run inside the container with manip_challenge (Gazebo) up and nothing else of ours running. |
